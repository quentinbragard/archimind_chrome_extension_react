...
const StatItem: React.FC<StatItemProps> = ({ icon, value, label, tooltip, unit }) => {
  return (
    <div 
      className="relative flex items-center justify-center h-10 mx-1 px-3 bg-primary/5 rounded-full transition-colors hover:bg-primary/10" 
...
const StatsPanel: React.FC<StatsPanelProps> = ({ stats, isLoading = false }) => {
  return (
    <div
      ...
      className="fixed top-2 left-1/2 transform -translate-x-1/2 bg-white border border-primary/15 rounded-full text-sm w-80 h-10 shadow-md transition-all duration-300 z-50 overflow-hidden"
    >
      {isLoading ? (
